...
cbind(matrix= as.numeric(beta_hat2), lm=coef(lm(y~x)))
```
...
WHCC_beta_hat2 <- matrix(0, 
                         nrow=2, 
                         ncol = 2)

for (i in 1:n) {
  WHCC_X <- X[i, , drop=FALSE]
  WHCC_beta_hat2 <- WHCC_beta_hat2+ as.numeric(resid[i]^2)* t(WHCC_X) %*% WHCC_X
}

WHCC_Resid <- solve(t(X)%*% X)

White_Result <-WHCC_Resid %*% WHCC_beta_hat2%*%WHCC_Resid

SE_WHCC_Beta_hat2 <-sqrt(White_Result[2,2])
...
for (i in 1:M){
  x_1hat <- rnorm(n,mean=3,sd=6)
  x_2hat <-rnorm(n,mean=2,sd=4)
  epsilonhat <- rnorm(n,mean=0,sd=100)
